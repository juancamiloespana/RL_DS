...
BATCH_SIZE =  100 # Número de episodios por iteración
...
filter_full=[]
for iter_no, batch in tqdm(enumerate(iterate_batches(env, net, BATCH_SIZE))):
    ...
    if reward_m > -8150943 or iter_no == 10:
...
con=sql.connect('datos')
#resultados.to_sql('res_100b_100ep', con)

resultados.info()

res_ep=pd.read_sql('''select n_bat, n_ep, 
                   sum(reward_step)*-1 as costo_total, 
                   sum(costo_exeso) as costo_exceso,
                   sum(costo_envio) as costo_envio,
                   sum(costo_BodExt) as costo_BE,
                   sum(costo_venta_per) as costo_VP
                   from res_100b_100ep
                    where n_bat<=50
                   group by n_bat, n_ep
...
res_ep.to_excel("resultados\\resultas_por_episodio.xlsx")
...
sns.pointplot(x='n_bat', y='costo_total', data=res_ep, errorbar='sd', color='darkgreen')

plt.title('Average total cost per episode', fontsize=16, fontweight='bold')
plt.xlabel('Batch', fontsize=14)
plt.ylabel('Average cost per episode', fontsize=14)
plt.show()
...
best_ep=pd.read_sql('''select * from res_100b_100ep
                    where n_bat<=30 and n_ep=33
                                   
                   
                   ''', con)

best_ep.to_excel("resultados\\best_ep.xlsx")